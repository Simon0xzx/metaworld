...
from metaworld.envs.mujoco.env_dict import SPECIAL_MODE_ARGS_KWARGS, SPECIAL_MODE_CLS_DICT


class MLSP(MultiClassMultiTaskEnv, Benchmark):

    def __init__(self, env_type='train', sample_all=False, task_name=None):
        assert env_type == 'train' or env_type == 'test'

        if task_name is not None:
            if task_name not in SPECIAL_MODE_CLS_DICT[env_type]:
                raise ValueError("{} does not exist in ML10 {} tasks".format(
                    task_name, env_type))
            cls_dict = {task_name: SPECIAL_MODE_CLS_DICT[env_type][task_name]}
            args_kwargs = {task_name: SPECIAL_MODE_ARGS_KWARGS[env_type][task_name]}
        else:
            cls_dict = SPECIAL_MODE_CLS_DICT[env_type]
            args_kwargs = SPECIAL_MODE_ARGS_KWARGS[env_type]
    ...
    def from_task(cls, task_name):
        if task_name in SPECIAL_MODE_CLS_DICT['train']:
            return cls(env_type='train', sample_all=True, task_name=task_name)
        elif task_name in SPECIAL_MODE_CLS_DICT['test']:
